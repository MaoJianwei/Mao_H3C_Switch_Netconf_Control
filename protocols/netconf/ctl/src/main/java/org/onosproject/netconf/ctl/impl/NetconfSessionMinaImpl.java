...
public class NetconfSessionMinaImpl implements NetconfSession {
    ...
    public void checkAndReestablish() throws NetconfException {
        try {
            if (client.isClosed()) {
                log.info("Trying to restart the whole SSH connection with {}", deviceInfo.getDeviceId());
                cleanUp();
                startConnection();
            } else if (session.isClosed()) {
                log.info("Trying to restart the session with {}", session, deviceInfo.getDeviceId());
                cleanUp();
                startSession();
            } else if (channel.isClosed()) {
                log.info("Trying to reopen the channel with {}", deviceInfo.getDeviceId());
                cleanUp();
                openChannel();
            } else {
                return;
    ...
    public class NetconfSessionDelegateImpl implements NetconfSessionDelegate {
        ...
        public void notify(NetconfDeviceOutputEvent event) {
            ...
            log.info("messageID {}, waiting replies messageIDs {}", messageId,
                    replies.keySet());
